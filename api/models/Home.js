...
module.exports = {
    save: function (data, callback) {
        if (!data._id) {
            ...
            sails.query(function (err, db) {
                ...
                db.collection('home').insert(data, function (err, created) {
                    if (err) {
                        console.log(err);
                        callback({
                            value: false
            ...
            sails.query(function (err, db) {
                var home = sails.ObjectID(data._id);
                delete data._id
                if (err) {
                    console.log(err);
                    callback({
    ...
    counthome: function (data, callback) {
        sails.query(function (err, db) {
            if (err) {
                console.log(err);
                callback({
            ...
            if (db) {
                db.collection("home").count({}, function (err, number) {
                    if (number != null) {
                        callback(number);
                    }
                });
